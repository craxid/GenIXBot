...
let handler = async (m, { conn, args, text, usedPrefix, command }) => {
   ...
   let jeson = await fetch(`https://vihangayt.me/search/sticker?q=${text}`)
...
▢ *Total stickers:* ${json.data.sticker_url.length}
▢ *Time:* _*${json.data.sticker_url.length * 2} s*_`)
    for (let i of json.data.sticker_url) {
