...
const connectionOptions = {
        ...
        printQRInTerminal: true,
        auth: state,
        browser: ['Akebi-chan', 'Safari', '3.1.0'], 
...
global.reloadHandler = async function (restatConn) {
  ...
  conn.spromote = '@user diangkat jadi admin'
  conn.sdemote = '@user diturunkan jabatannya'
  conn.sDesc = 'Deskripsi grup diubah menjadi \n@desc'
  conn.sSubject = 'Nama grup diubah menjadi \n@group'
  conn.sIcon = 'Ikon grup diubah'
  conn.sRevoke = 'Tautan grup diubah menjadi \n@revoke'
...
global.reload = async (_ev, filename) => {
  if (pluginFilter(filename)) {
    ...
    if (filename in global.plugins) {
      if (existsSync(dir)) conn.logger.info(`🌟 Plugin diperbarui - '${filename}'`)
      else {
