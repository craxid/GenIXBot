        ...
        for (let name in global.plugins) {
            ...
            if ((usedPrefix = (match[0] || '')[0])) {
                ...
                if (!isPrems && plugin.diamond && global.db.data.users[m.sender].diamond < plugin.diamond * 1) {
                    this.reply(m.chat, `✳️ Diamond kamu habis\ngunakan perintah berikut untuk membeli lebih banyak berlian \n*${usedPrefix}buy* <jumlah> \n*${usedPrefix}buyall*`, m)
                    continue // Limit habis
                }
                if (plugin.level > _user.level) {
                    this.reply(m.chat, `✳️ Level ${plugin.level} dibutuhkan untuk menggunakan perintah ini. \nLevel kamu ${_user.level}`, m)
                ...
                try {
                    ...
                    if (m.diamond)
                        m.reply(`Diamond kamu dikurangi *${+m.diamond}* 💎`)
                }
                break
            }
        }
...
let wel = API(`https://api.azz.biz.id/api/welcome?name=${name}&gcname=Miftah%20Botz&member=${member}&pp=${pp}&bg=https://minimalistic-wallpaper.demolab.com/?random&key=global`)

let lea = API(`https://api.azz.biz.id/api/welcome?name=${name}&gcname=Miftah%20Botz&member=${member}&pp=${pp}&bg=https://minimalistic-wallpaper.demolab.com/?random&key=global`)
...
this.sendFile(id, action === 'add' ? wel : lea, 'pp.jpg', text, null, false, { mentions: [user] })
        ...
        case 'demote':
            ...
            if (!text)
                text = (chat.sDemote || this.sdemote || conn.sdemote || 'jabatan @user diturunkan')
...
export async function groupsUpdate(groupsUpdate) {
    ...
    for (const groupUpdate of groupsUpdate) {
        ...
        if (groupUpdate.desc) text = (chats.sDesc || this.sDesc || conn.sDesc || 'Deskripsi grup diubah menjadi \n@desc').replace('@desc', groupUpdate.desc)
        ...
        if (groupUpdate.icon) text = (chats.sIcon || this.sIcon || conn.sIcon || 'Ikon grup diubah menjadi').replace('@icon', groupUpdate.icon)
...
watchFile(file, async () => {
    unwatchFile(file)
    console.log(chalk.magenta("✅ Mengupdate 'handler.js'"))
